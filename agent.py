...
class Agent(nn.Module):
    ...
    def predict_lanes_test_quantized(self, im):
        im = torch.from_numpy(im).to(torch.device('cuda:0'))
        self.binding_addrs['input.1'] = int(im.data_ptr())
        self.context.execute_v2(list(self.binding_addrs.values()))
        y = [self.bindings[x].data for x in self.output_names]
        outputs = [y[0:3], y[3:6], y[6:9], y[9:12]]
        #print(self.output_names)
        #for output_value in y:
            #print (output_value.shape)
        return outputs

    def predict_lanes_test_quantized2(self, im):
        if self.stream is None:
            print(im.shape)
            batch = torch.from_numpy(im).float().to(torch.device('cuda:0'))
            outputs, features = self.lane_detection_network(batch)
            #print(outputs)
            self.d_input = cuda.mem_alloc(1 * im.nbytes)

            self.outputs = [None] * len(outputs)
            self.d_outputs = [None] * len(outputs)
            self.features = [None] * len(features)
            self.d_features = [None] * len(features)

            for i in range(len(outputs)):
                self.outputs[i] = [
                    np.empty(outputs[i][0].size(), dtype = np.float16),
                    np.empty(outputs[i][1].size(), dtype = np.float16),
                    np.empty(outputs[i][2].size(), dtype = np.float16)
                ]
                self.d_outputs[i] = [
                    cuda.mem_alloc(1 * self.outputs[i][0].nbytes),
                    cuda.mem_alloc(1 * self.outputs[i][1].nbytes),
                    cuda.mem_alloc(1 * self.outputs[i][2].nbytes)
                ]
            for i in range(len(features)):
                self.features[i] = np.empty(features[i].size(), dtype = np.float16)
                self.d_features[i] = cuda.mem_alloc(1 * self.features[i].nbytes)
            
            self.bindings = [
                int(self.d_input),
                int(self.d_outputs[0][0]), int(self.d_outputs[0][1]), int(self.d_outputs[0][2]),
                int(self.d_outputs[1][0]), int(self.d_outputs[1][1]), int(self.d_outputs[1][2]),
                int(self.d_outputs[2][0]), int(self.d_outputs[2][1]), int(self.d_outputs[2][2]),
                int(self.d_outputs[3][0]), int(self.d_outputs[3][1]), int(self.d_outputs[3][2]),
                int(self.d_features[0]), int(self.d_features[1]), int(self.d_features[2]), int(self.d_features[3])
            ]
        ...
        for i in range(len(self.d_outputs)):
            for j in range(len(self.d_outputs[i])):
                cuda.memcpy_dtoh_async(self.outputs[i][j], self.d_outputs[i][j], self.stream)
        # Syncronize threads
        self.stream.synchronize()
        print(self.outputs[0][0])
        return self.outputs
    #####################################################
    ## Training mode
    #####################################################                                                
    ...
    def load_weights_quantized(self):
        w = './pinet.engine'
        ...
        Binding = namedtuple('Binding', ('name', 'data', 'ptr'))
        self.feature_names = []
        self.output_names = []
        for i in range(model.num_bindings):
            ...
            if model.binding_is_input(i):
                print(name)
            elif name in ['input.140', 'input.536', 'input.932', 'input.1328']:  # features
                self.feature_names.append(name)
            else:  # output
                self.output_names.append(name)
            shape = tuple(self.context.get_binding_shape(i))
            im = torch.from_numpy(np.empty(shape, dtype=dtype)).to(torch.device('cuda:0'))
            self.bindings[name] = Binding(name, im, int(im.data_ptr()))
        self.binding_addrs = OrderedDict((n, d.ptr) for n, d in self.bindings.items())
        print(self.binding_addrs)

    def load_weights_quantized2(self):
        w = './pinet.engine'
    ...
    def export_tensorrt(self, input_image, filename):
        import tensorrt as trt
        import os
        print('first, converting to onnx model...')

        onnx_file = filename.replace('.engine','.onnx')
        if not os.path.exists(onnx_file):
            torch.onnx.export(self.lane_detection_network, input_image, onnx_file, verbose=False, opset_version=12)
        else:
            print('onnx_file already found. skip onnx conversion')

        print('now, converting to tensorrt model ...')
        ...
        with builder.build_engine(network, config) as engine, open(filename, 'wb') as t:
